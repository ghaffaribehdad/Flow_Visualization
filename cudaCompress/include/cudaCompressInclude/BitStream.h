...
inline uint BitStreamReadOnly::readBits(uint& data, uint bitCount)
{
    ...
    uint bitCountRead = std::min<uint>(bitCount, m_bitSize - getBitPosition());

    bitCount = bitCountRead;
    //TODO optimize? this needs at most 2 iterations...
    while(bitCount > 0) {
        // number of bits to read from current uint
        uint bitCountCur = std::min<uint>(bitCount, 32 - m_bitPos);
...
{
    uint bitCountSkipped = std::min<uint>(bitCount, m_bitSize - getBitPosition());
...
inline uint BitStreamReadOnly::readAligned(T* pTarget, uint count)
{
    ...
    uint elemsRead = std::min<uint>(elemsAvailable, count);
...
inline void BitStream::setBitSize(uint bitSize)
{
    ...
    bitPos = std::min<uint>(bitPos, m_bitSize);
