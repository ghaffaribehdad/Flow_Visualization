...
void decompressVolumeFloat(GPUResources& shared, CompressVolumeResources& resources, float* dpImage, uint sizeX, uint sizeY, uint sizeZ, uint numLevels, const std::vector<uint>& bitStream, float quantizationStepLevel0, size_t bufferSize, bool doRLEOnlyOnLvl0 = false);
