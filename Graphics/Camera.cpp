...
void Camera::SetParallelProjectionValues(const float & aspectRatio, const float & viewHeight, float nearZ, float farZ)
{
	this->parallelProjectionMatrix = XMMatrixOrthographicLH(2.0f* aspectRatio, 2, nearZ, farZ);
...
const XMMATRIX & Camera::GetParallelProjectionMatrix() const
{
	return this->parallelProjectionMatrix;
}
...
void Camera::SetLookAtPos(XMFLOAT3 lookAtPos)
{
	...
	if (lookAtPos.y != 0.0f)
	{
		const float distance = static_cast<float>(sqrt(lookAtPos.x * lookAtPos.x + lookAtPos.z * lookAtPos.z));
		pitch = static_cast<float>(atan(lookAtPos.y / distance));
	}

	float yaw = 0.0f;
	if (lookAtPos.x != 0.0f)
	{
		yaw = static_cast<float>(atan(lookAtPos.x / lookAtPos.z));
	}
	if (lookAtPos.z > 0)
		yaw += XM_PI;
...
const XMVECTOR& Camera::GetUpXMVector()
{
...
const XMVECTOR& Camera::GetViewXMVector()
{
