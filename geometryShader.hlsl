...
[maxvertexcount(18)]
void main(line VS_OUTPUT input[2], inout TriangleStream<GS_OUTPUT> output) {

     float StripWidth = LineRadius *input[0].size;
  ...
  if (input[0].lineID == input[1].lineID)
     {
         ...
         float3 viewDirection = normalize(input[0].position.xyz - CameraPosition);
